...
global $returnList;
  ...
  if($date!=null && $sender!=null) $mwhere = " AND m.dateSent > " . $date . " AND m.sender= ".$sender;
    ...
    $status=mysql_result($result,$i,"readStatus");
		...
		$query2="SELECT m.id, m.sender, m.message, m.conversationId, m.dateSent FROM message as m WHERE m.id=" . $id . $mwhere;
    ...
    $sender=mysql_result($result2,0,"sender");
	   ...
	   $dateSent=mysql_result($result2,0,"dateSent");



    $text=mysql_result($result2,0,"message");
    
    $convoId=mysql_result($result2,0,"conversationId");
    $indMessId=mysql_result($result2,0,"id");
   ...
   // echo "<br />Sent by: ".$first."<br />Message: ".$text."<br />Sent To: ";
		...
		$returnList[$i][8] = $convoId;
		
		$returnList[$i][9] = $indMessId;
		
		$returnList[$i][10] = $dateSent;	
...
function getEmployeeThumbnail($employeeID){

	$query="SELECT e.thumb FROM employees as e WHERE e.id=".$employeeID;

	$result=mysql_query($query);

	return mysql_result($result, 0, "thumb");
...

{
	...
	$num=mysql_numrows($result2);
		...
		while ($i < $num) {
			...
			$returnConvo[$i][0] = getEmployeeName($sender);
			$returnConvo[$i][1] = getDevice(1, $device);
			$returnConvo[$i][2] = $message;
			$returnConvo[$i][3] = $recip;
			$returnConvo[$i][4] = $sender;
